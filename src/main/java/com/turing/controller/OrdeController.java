...
public class OrdeController {
    ...
    public String userAddCar(@RequestParam("productId") Integer productId, HttpServletResponse response,
        ...
        Car carInfo = new Car();
        //查询库存
        Product pinit = new Product();
        pinit.setId(productId);
        ...
        pinit = productService.selectProductInfoById(pinit);
        if (pinit.getStock() < 5) {
            out = response.getWriter();
            out.print("<script>alert('亲~，库存不足，添加购物车失败!');" +
                    "</script>");
            out.flush();
        ...
        if (rs == 1) {
            out = response.getWriter();
            out.print("<script>alert('亲~，添加购物车成功!');" +
                    "</script>");
            out.flush();
            return "user/product-view";
        } else {
            out = response.getWriter();
            out.print("<script>alert('亲~，添加购物车失败!');" +
                    "</script>");
            out.flush();
            return "user/product-view";
        }
